...
@property (nonatomic, weak)   id<JXImageLoader>               imageLoader;
...
- (void)setJxImage:(JXImage *)jxImage {
    ...
    if (jxImage.imageMax) {
        ...
        if(self.imageLoader) {
            [self.imageLoader downloadImageWithURL:self.jxImage.urlImg progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            ...
            } completed:^(UIImage * _Nonnull image, NSError * _Nonnull error, BOOL finished) {
                if (image && finished) {
...
@property (nonatomic, strong)   id<JXImageLoader>               imageLoader;
...
+ (void)browseImages:(NSArray <JXImage *> *)images fromIndex:(NSInteger)fromIndex imageLoader:(nonnull id<JXImageLoader>)loader{
    ...
    imageBrowser.imageLoader = loader;
...
- (void)createComponents {
    ...
    for (NSInteger i = 0; i < imageViewCount; i ++) {
        ...
        jxImageView.imageLoader = self.imageLoader;
